...
export function AuditLogDisplay({ auditLogs }: AuditLogDisplayProps) {
    if (auditLogs.length === 0) {
        return <p className="text-muted-foreground text-center">Δεν υπάρχει ακόμη ιστορικό ενεργειών για αυτό το έργο.</p>
    }
  return (
    <Table>
      <TableHeader>
      ...
      <TableBody>
        {auditLogs.map((log) => (
          <TableRow key={log.id}>
            ...
            <TableCell className="text-muted-foreground">{log.details || "Δ/Υ"}</TableCell>
