...
import { type Project, type ProjectIntervention, type Stage, type Unit, type ExpenseCategory } from '@/types';
...
export async function createProjectAction(prevState: any, formData: FormData) {
    ...
    // Διορθώθηκε η τιμή του status σε 'On Track' για να συμφωνεί με τον τύπο δεδομένων Project
...
export async function updateInterventionAction(prevState: any, formData: FormData) {
    ...
    // Πρέπει να ανακτήσουμε το master intervention για να υπολογίσουμε το maxUnitPrice/maxAmount
...
export async function createMasterInterventionAction(prevState: any, formData: FormData) {
    ...
    try {
        const db = getAdminDb();
        const success = await addMasterIntervention(db, id, data);
